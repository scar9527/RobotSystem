...
void Form_ScaraArm::on_ScaraArm_btn_PosGo_clicked()
{
	...
	CScaraArm->TrajectoryPlanning(oz, px, py);
}
...
void Form_ScaraArm::Display()
{
	...
	// forward kinematics
	ui->ScaraArm_label_PresentX->setText(QString::number((int)CScaraArm->GetCurrentPosition(0)));
	ui->ScaraArm_label_PresentY->setText(QString::number((int)CScaraArm->GetCurrentPosition(1)));
	ui->ScaraArm_label_PresentOz->setText(QString::number((int)CScaraArm->GetCurrentOrientation(2)));
